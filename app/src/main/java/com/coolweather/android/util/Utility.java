        ...
        if(!TextUtils.isEmpty(response)) {
            try {
                ...
                for (int i = 0; i < allProvinces.length(); i++) {
                    JSONObject prvinceObject = allProvinces.getJSONObject(1);
        ...
        if(!TextUtils.isEmpty(response)) {
            try {
                ...
                for (int i = 0; i < allCities.length(); i++) {
                    JSONObject cityObject = allCities.getJSONObject(1);
                    City city = new City();
        ...
        if(!TextUtils.isEmpty(response)) {
            try {
                ...
                for (int i = 0; i < allCounties.length(); i++) {
                    JSONObject countyObject = allCounties.getJSONObject(1);
